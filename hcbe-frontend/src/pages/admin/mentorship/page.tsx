...
const Badge = ({ status, label }: { status: string; label: string }) => <span className={`rounded-full border px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.14em] ${tones[status] || tones.Completed}`}>{label}</span>;
...
const AdminMentorshipPage = () => {
  ...
  const c = fr ? {
    ...
    profile: 'Profil', expertise: 'Expertise', goals: 'Objectifs',
    statusPending: 'À examiner', statusApproved: 'Approuvé', statusRejected: 'Refusé', statusWithdrawn: 'Retiré',
    statusActive: 'Actif', statusProposed: 'Proposé', statusDeclined: 'Décliné', statusCompleted: 'Terminé', statusCancelled: 'Annulé',
    ...
    profile: 'Profile', expertise: 'Expertise', goals: 'Goals',
    statusPending: 'Pending', statusApproved: 'Approved', statusRejected: 'Rejected', statusWithdrawn: 'Withdrawn',
    statusActive: 'Active', statusProposed: 'Proposed', statusDeclined: 'Declined', statusCompleted: 'Completed', statusCancelled: 'Cancelled',
    ...
    <div className="space-y-6">
      ...
      <div className="grid items-start gap-6 2xl:grid-cols-[minmax(0,1.35fr)_minmax(360px,.65fr)]">
        <section className="overflow-hidden rounded-[18px] border border-line bg-surface">
          <div className="border-b border-line px-5 py-4"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[9px] font-bold uppercase tracking-[.18em] text-red-link">{c.applications}</p><h2 className="mt-1 font-display text-2xl font-bold text-green-deep">{c.pending}</h2></div><div className="grid w-full gap-2 sm:w-auto sm:grid-cols-3"><input value={search} onChange={(e) => setSearch(e.target.value)} className={inputClasses} placeholder={c.search} /><select value={role} onChange={(e) => setRole(e.target.value)} className={inputClasses}><option value="">{c.allRoles}</option><option value="Mentor">{c.mentor}</option><option value="Mentee">{c.mentee}</option></select><select value={status} onChange={(e) => setStatus(e.target.value)} className={inputClasses}><option value="">{c.allStatuses}</option><option value="Pending">{c.statusPending}</option><option value="Approved">{c.statusApproved}</option><option value="Rejected">{c.statusRejected}</option><option value="Withdrawn">{c.statusWithdrawn}</option></select></div></div></div>
          <div className="divide-y divide-line">{loading ? <div className="py-16 text-center text-ink-variant"><i className="ri-loader-4-line animate-spin text-2xl" /></div> : filteredApplications.length === 0 ? <p className="p-8 text-center text-sm text-ink-variant">{c.empty}</p> : filteredApplications.map((item) => <article key={item.id} className="p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-display text-xl font-bold text-green-deep">{item.memberName}</h3><Badge status={item.status} label={statusLabel(item.status)} /></div><p className="mt-1 text-xs text-ink-variant">{item.memberEmail} · {item.role === 'Mentor' ? c.mentor : c.mentee} · {item.preferredLanguage.toUpperCase()}</p></div><span className="rounded-full bg-canvas px-3 py-1 text-[10px] font-semibold text-ink-variant">{new Date(item.createdAt).toLocaleDateString(fr ? 'fr-CA' : 'en-CA')}</span></div><div className="mt-4 grid gap-4 md:grid-cols-3"><div><p className="text-[9px] font-bold uppercase tracking-[.14em] text-ink-variant">{c.profile}</p><p className="mt-1 text-sm leading-5 text-ink-variant">{item.professionalSummary}</p></div><div><p className="text-[9px] font-bold uppercase tracking-[.14em] text-ink-variant">{c.expertise}</p><p className="mt-1 text-sm leading-5 text-ink-variant">{item.expertise}</p></div><div><p className="text-[9px] font-bold uppercase tracking-[.14em] text-ink-variant">{c.goals}</p><p className="mt-1 text-sm leading-5 text-ink-variant">{item.objectives}</p></div></div>{item.status === 'Pending' && <div className="mt-4 flex flex-col gap-3 border-t border-line pt-4 sm:flex-row sm:items-end"><div className="flex-1"><Field label={c.note} htmlFor={`note-${item.id}`}><input id={`note-${item.id}`} className={inputClasses} value={reviewNotes[item.id] || ''} onChange={(e) => setReviewNotes({ ...reviewNotes, [item.id]: e.target.value })} /></Field></div><div className="flex gap-2"><Button type="button" variant="primary" disabled={busy} onClick={() => void run(() => communityApi.adminReview(item.id, 'Approved', reviewNotes[item.id]), c.saved)}>{c.approve}</Button><Button type="button" variant="tertiary" disabled={busy} onClick={() => void run(() => communityApi.adminReview(item.id, 'Rejected', reviewNotes[item.id]), c.saved)}>{c.reject}</Button></div></div>}{item.committeeNotes && item.status !== 'Pending' && <p className="mt-4 border-l-2 border-gold pl-3 text-xs text-ink-variant">{item.committeeNotes}</p>}</article>)}</div>
        </section>
