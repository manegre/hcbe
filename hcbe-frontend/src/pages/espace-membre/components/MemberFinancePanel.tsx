...
export default function MemberFinancePanel({ member }: { member: MemberDto }) {
  ...
  return <div className="space-y-7" data-testid="member-finance-panel">
    ...
    {valid && <section className={`grid gap-3 sm:grid-cols-2 ${wallet?.enabled ? 'xl:grid-cols-4' : ''}`} aria-label={fr ? 'Actions de la carte membre' : 'Membership card actions'}>
