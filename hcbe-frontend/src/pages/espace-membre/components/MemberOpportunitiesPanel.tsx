  ...
  return <div className="space-y-8">
    ...
    <section aria-labelledby="opportunity-list-title">
      ...
      {visible.length === 0 ? <div className="mt-5 rounded-[24px] border border-dashed border-line bg-surface p-10 text-center text-sm text-ink-variant">{copy.empty}</div> : <div className="mt-5 grid gap-5 lg:grid-cols-2">{visible.map(({ opportunity: item, score, reasons }) => <article key={item.id} className="group flex min-w-0 flex-col overflow-hidden rounded-[24px] border border-line bg-surface shadow-[0_14px_40px_rgba(0,59,27,.06)] transition hover:-translate-y-0.5 hover:border-green/30">
        ...
        <div className="flex flex-1 flex-col p-5"><p className="line-clamp-4 text-sm leading-6 text-ink-variant">{localized(item.description, item.descriptionEn, i18n.language)}</p>{reasons.length > 0 && <div className="mt-4"><p className="text-[9px] font-bold uppercase tracking-[.14em] text-ink-muted">{copy.why}</p><div className="mt-2 flex flex-wrap gap-2">{reasons.map((reason) => <span key={reason} className="rounded-lg bg-green/5 px-2.5 py-1.5 text-xs font-semibold text-green"><i className="ri-sparkling-line mr-1 text-gold-dark" />{reasonLabel(reason)}</span>)}</div></div>}
        ...
        {(item.requirements || item.benefits) && <details className="mt-4 rounded-xl bg-canvas/60 p-3 text-sm"><summary className="cursor-pointer font-bold text-green-deep">{en ? 'Full details' : 'Tous les détails'}</summary>{item.requirements && <p className="mt-3 leading-6 text-ink-variant"><strong>{copy.requirements}:</strong> {localized(item.requirements, item.requirementsEn, i18n.language)}</p>}{item.benefits && <p className="mt-2 leading-6 text-ink-variant"><strong>{copy.benefits}:</strong> {localized(item.benefits, item.benefitsEn, i18n.language)}</p>}</details>}
