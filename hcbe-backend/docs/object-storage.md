...
Before removing the legacy Fly volume, copy existing `/app/data/uploads` objects into the bucket under the `hcbe` prefix and update stored database URLs to `/api/storage/hcbe/...`. Verify a sample of images and documents from the public site, then retain a volume snapshot through the agreed rollback window.
