...
public sealed class OpportunityService(ApplicationDbContext context, INotificationService notifications) : IOpportunityService
{
    ...
    private static OpportunityApplicationDto MapApplication(OpportunityApplication item) => new(item.Id, item.OpportunityId, item.Opportunity?.Title ?? "", item.Opportunity?.TitleEn, item.MemberId, $"{item.Member?.FirstName} {item.Member?.LastName}".Trim(), item.Member?.Email ?? "", item.Message, item.Status, item.AdminNotes, item.CreatedAt, item.UpdatedAt);
